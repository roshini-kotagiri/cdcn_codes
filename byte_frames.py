...
def byte_destuffing(data): 
    ...
    while i < len(words): 
        if words[i] == ESC: 
            # Check if the next word is FLAG or ESC 
...
def main(): 
    ...
    data = input("Enter the sentence (words separated by spaces): ") 
    frame_size = int(input("Enter the size of each frame (e.g., 10): ")) 
 
    # Perform byte stuffing 
    stuffed_data = byte_stuffing(data) 
 
    # Split stuffed data into frames 
    print("\nSender Side:") 
    print(f"User Data: {data}") 
    print(f"Byte Stuffed: {stuffed_data}") 
 
    print(f"\nData Sent in Frames (size {frame_size}):") 
    words = stuffed_data.split() 
    for i in range(0, len(words), frame_size): 
        print(f"Frame: {' '.join(words[i:i + frame_size])}") 
 
    # Simulate transmission 
    received_data = stuffed_data 
 
    # Perform byte destuffing 
    destuffed_data = byte_destuffing(received_data) 
